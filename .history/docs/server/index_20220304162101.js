...
app.post("/api/auth/signup", (req, res) => {
    ...
    const corper = {
        ...
        password: bcrypt.hash(req.body.statecode, 10),
...
app.post("/api/auth/signin", async (req,res) => {
    ...
    const passwordIsValid = bcrypt.hash(password);
    dbConnect.query("SELECT * FROM corpers WHERE password = ?", passwordIsValid, (err,result) =>{
        if(err){
            console.log(err);
        } else {
            console.log(result);
