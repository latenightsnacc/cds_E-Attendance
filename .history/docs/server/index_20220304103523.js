...
app.post("/api/auth/signin", async (req,res) => {
    ...
    if(email.length > 0){
        corper = dbConnect.query(`SELECT * FROM corpers WHERE email = ? AND  `,[email, passwordIsValid], (err,result) => {
            if(err) {
                console.log(err)
            } else {
                res.send(result);
        ...
        //corper = dbConnect.query(`SELECT * FROM corpers WHERE email = ? `,email);
        console.log(corper);
        // if(corper){
        //     
