...
const pool = mysqlStore
dbConnect.connect(err => {
    if(err) {
        console.log(err)
    } else {
        console.log('Database connected successfully!');
        sessionStore = new mysqlStore(dbConnect, dbConfig.pool);
    }
})
// const sessionStore = new mysqlStore(db.sequelize, dbConfig.pool);
app.use(session({
...
if(session.userid){
    ...
    app.get(`/api/dashboard/member/`, (req, res) => {
        session=req.session;
        db.corpers.findOne(corperSession.id).then(response => {
            res.send(response);
        }) 
    })
}
