...
app.post("/api/auth/signin", redirectDasboard, async (req,res) => {
    try{
        ...
        pool.query("CREATE TABLE IF NOT EXISTS `sessions` (`session_id` varchar(128) COLLATE utf8mb4_bin NOT NULL,`expires` int(11) unsigned NOT NULL,`data` mediumtext COLLATE utf8mb4_bin,PRIMARY KEY (`session_id`)
          ) ENGINE=InnoDB
        req.session.userId = corper.id;
        res.send(`/api/dashboard/member/${corper.statecode}`);
...
app.get(`/api/dashboard/member/:id`, redirectLogin, async (req, res) => {
    ...
    if(userId){
        try{
            const corper = await db.getCorperById(userId);
            console.log(corper);
            res.send(corper);
        }catch(e){
            console.log(e);
