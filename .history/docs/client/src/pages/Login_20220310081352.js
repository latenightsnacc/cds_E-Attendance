...
const Login = () => {
    const { state: {userDetails, auth, loading, errorMessage}, dispatch } = useContext(UserContext);
    ...
    const handleLogin = e => {
        e.preventDefault();
        let user = dispatch({type: "REQUEST_LOGIN"}) ? dispatch({type: "LOGIN_SUCCESSFUL"}) : null;
            ...
            <span className="text-green-500 font-semibold capitalize">CDS E-attendance</span>
                ...
                <div className="w-full max-w-sm">
                    <div className="shadow-2xl text-left w-full text-xs p-2">
                        <p>Authentication: {state.auth}</p>
                        <p>User: {state.userDetails}</p>
                </div>
            
            {
                state.errorMessage ? <div className="msg mt-2 hidden">
                <p className="text-red-500 text-xs italic">{state.errorMessage}</p>
                </div> : null
            }
                ...
                <div className="flex w-full flex-col">
                    ...
                    <button id="loginBtn" onClick={handleLogin}  disabled={state.loading} className="bg-transparent hover:bg-transparent text-black font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="submit">
