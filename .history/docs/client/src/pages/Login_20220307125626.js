...
import { loginUser, useAuthState, useAuthDispatch } from "../auth/actions.context";
...
const Login = (props) => {
    ...
    const {loading, errorMessage } = use
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        let payload = {firstname,surname,batch,statecode,cdsGroup,lga,ppa,phoneNo,email,profilePic,roles,status,state}
        try {
            let response = await loginUser(dispatch, payload)
            if (!response.user) return
            props.history.push('/dashboard/member')
        }catch (error) {
            console.log(error);
        }
    }
        ...
        <Container>
            ...
            <span className="text-green-500 font-semibold capitalize">CDS E-attendance</span>
                ...
                <div className="flex w-full flex-col">
                    ...
                    <button id="loginBtn" onClick={handleLogin} className="bg-transparent hover:bg-transparent text-black font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="submit">
